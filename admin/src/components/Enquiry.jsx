...
const Enquiry = () => {
  ...
  useEffect(() => {
    handleSearch();
  }, [searchTerm, selectedStatus, users]); // Add users as dependency to re-filter when it updates
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "cancelled":
        return "text-gray-500"; // Customize this color if needed
      default:
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex justify-center mb-4">
        {["all", "pending", "Processing", "confirm", "cancelled"].map((status) => (
          <button
            key={status}
            onClick={() => {
          ...
          {currentUsers.length > 0 ? (
            currentUsers.map((user) => (
              <tr key={user._id}>
                ...
                <td className="py-2 px-4 border-b flex flex-col justify-center items-center">
                  <div>
                    <img src={`${URI}uploads/${user.productImage}`} alt="" className="w-32" />
                  </div>
                  <div className="flex flex-col items-center">
                    <p className="truncate">{user.productName}</p>
                    <Link to={`/product/${user.productId}`} className="text-indigo-600">
                      View Product
                    </Link>
                  </div>
                </td>
                <td className={`py-2 px-4 border-b ${getStatusColor(user.status)}`}>
                  {user.status}
                </td>
                ...
                {user.status !== "confirm" && (
                  <td className="py-2 px-4 border-b">
                    ...
                    >
                      <option value="pending" disabled={user.status === "Processing"}>
                        Pending
                      </option>
